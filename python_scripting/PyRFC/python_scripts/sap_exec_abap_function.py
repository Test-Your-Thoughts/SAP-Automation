...
def call_rfc(function, parameters, connection):
    """Call an SAP RFC function."""

    conn = Connection(**connection)

    try:
        result = conn.call(function, **parameters)
        return result
...
def main():
    parser = argparse.ArgumentParser(
        description="Execute an SAP RFC using PyRFC."
    )

    parser.add_argument("--ashost", required=True)
    ...
    parser.add_argument(
        "--parameters",
        required=True,
        help="JSON string containing RFC parameters",
    )
    ...
    try:
        ...
        print(f"Invalid JSON for --parameters: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"Logon error: {e}", file=sys.stderr)
        sys.exit(3)

    except (ABAPApplicationError, ABAPRuntimeError) as e:
        print(f"ABAP error: {e}", file=sys.stderr)
        sys.exit(4)
